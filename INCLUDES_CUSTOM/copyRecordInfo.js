function copyRecordInfo(fromCapId, toCapId, infoToCopy) {
    ...
    ************************************************************/
	if (!fromCapId || !toCapId) {
		logDebug("From or to capID is null");
		return;
	}
			...
			switch ("" + infoType) {
				...
				case "ASI":
					...
					for (asi in AppSpecInfo) {
						...
						if (!ignore)
							editAppSpecific(asi, AppSpecInfo[asi], toCapId);
					}
					useAppSpecificGroupName = true;
					break;
				case "ASIT":
				...
				case "RECORDDETAILS":
					...
					if (!ccdiResult.getSuccess())
						logDebug("Error copying record details " + ccdiResult.getErrorMessage());
					ccwdiResult = aa.cap.copyCapWorkDesInfo(fromCapId, toCapId);
					if (!ccdwiResult.getSuccess())
						logDebug("Error copying record details " + ccdwiResult.getErrorMessage());
					break;
				case "CONDITIONS":
					...
					if (s_result.getSuccess()) {
						...
						for (var sIndex in sourceCapConditions) {
							...
							if (!createResult.getSuccess()) {
								logDebug("Error creating cap condition " + createResult.getErrorMessage());
							}
						}
					}
					break;
				case "ADDRESS":
					...
					if (addrsToRemove) {
						for (x in addrsToRemove)
							aa.address.removeAddress(toCapId, addrsToRemove[x].getAddressId());
					}
					...
					if (s_result.getSuccess()) {
						...
						if (bvaluatnScriptModel != null) {
							...
							if (capDetail != null) {
								...
								if (!editResult.getSuccess())
									logDebug("Error copying additional info " + editResult.getErrorMessage());
							}
						}
					}
					break;
				case "EDUCATION":
					...
					if (!copyResult.getSuccess())
						logDebug("Error copying education " + copyResult.getErrorMessage());
					break;
				case "CONTEDUCATION":
					copyResult = aa.continuingEducation.copyContEducationList(fromCapId, toCapId);
					if (!copyResult.getSuccess())
						logDebug("Error copying continuing education " + copyResult.getErrorMessage());
					break;
				case "EXAM":
					copyResult = aa.examination.copyExaminationList(fromCapId, toCapId);
					if (!copyResult.getSuccess())
						logDebug("Error copying examination info " + copyResult.getErrorMessage());
					break;
				case "DOCUMENT":
